...
def build_video(entries: List[Dict[str, str]], out_path: str) -> None:
    ...
    # Take latest 5 entries (by provided order; assuming chronological, use last 5)
    count = min(5, len(entries))
    indices = list(range(len(entries) - count, len(entries)))
    
    seg_dur = DURATION / max(1, len(indices))

    overlays = []
    for i, idx in enumerate(indices):
        e = entries[idx]
        prev = entries[idx - 1] if idx - 1 >= 0 else None
        panel = make_panel_clip(e, prev)
        panel = (
            panel.with_start(i * seg_dur)
                  .with_duration(seg_dur)
                  .with_position(("center", int(H * 0.14)))
    ...
    comp = CompositeVideoClip([bg, footer_clip, *overlays], size=(W, H))
